...
export class Launcher {
    ...
    public async launch<T>(count: number, conectorCreateFunction: (config: ConnectorConfig) => T): Promise<Awaited<T>[]> {
        const clients: Awaited<T>[] = [];
        const ports: number[] = [];

        for (let i = 0; i < count; i++) {
            const port = await getPort();
            clients.push(await conectorCreateFunction({ baseUrl: `http://localhost:${port}`, apiKey: this.apiKey }));
            ports.push(port);
