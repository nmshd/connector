...
export class FilesController extends BaseController {
    ...
    public async getFile(@PathParam("idOrReference") idOrReference: string, @ContextAccept accept: string, @ContextResponse response: express.Response): Promise<Envelope | void> {
        ...
        switch (accept) {
            case "image/png":
                return await this.qrCode(result, (r) => QRCode.from(r.value.truncatedReference, "tr"), `${fileId}.png`, response, 200);
            default:
                return this.ok(result);
        }
    }
    ...
    public async createTokenForFile(
        ...
        switch (accept) {
            case "image/png":
                return await this.qrCode(result, (r) => QRCode.from(r.value.truncatedReference, "tr"), `${id}.png`, response, 201);
            default:
                return this.created(result);
        }
    }
