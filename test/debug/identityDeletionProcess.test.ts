...
describe("Identity Deletion Process", () => {
    ...
    test("should start an identity deletion and get its status", async () => {
        const response = await axiosInstance.post<{ result: IdentityDeletionProcessDTO }>("/api/core/v1/IdentityDeletionProcess");

        expect(response.status).toBe(200);
        const identityDeletionProcess = response.data.result;
        expect(identityDeletionProcess.status).toBe("Active");
    ...
    test("should get the active identity deletion process", async () => {
        const initiateResult = await axiosInstance.post<{ result: IdentityDeletionProcessDTO }>("/api/core/v1/IdentityDeletionProcess");
        expect(initiateResult.status).toBe(200);
        const identityDeletionProcess = initiateResult.data.result;

        const getResult = await axiosInstance.get<{ result: IdentityDeletionProcessDTO }>("/api/core/v1/IdentityDeletionProcess");
